...
const SecondPage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setTotalTimeTaken({ minutes: totalTimeTakenMinutes, seconds: totalTimeTakenSeconds });
  };
    ...
    <div>
       {!showThirdPage && (<div >
        ...
        <h3 className="font-bold text-center text-xl">
          Given time is <span className="text-blue-900">{minutes}:{seconds < 10 ? `0${seconds}` : seconds}</span> to attempt all questions otherwise selected answers will be submitted automatically
        </h3>
          ...
          <form onSubmit={handleSubmit}> 
            <div className= 'text-2xl'>
              ...
              <input
                ...
                onChange={handleInputChange}
              />{" "}
              <label for="perform">To perform mathematical calculations</label>
              <br />
              <br />
            ...
            <button type="submit" className="rounded-lg bg-blue-500 text-2xl w-40 pb-3 pt-3 text-white">Submit</button>
          </form>
        </div>
      </div>)}
      {showThirdPage && (
        <ThirdPage selectedValues={selectedValues}  totalTimeTaken={totalTimeTaken} />
      )}
    </div>
  );
};
